...
- **Emotion / Style（instruct 字段）** — 引擎把它当作"对说话人的一句指令"分词，所以必须写**声音该怎么做**，而不是这一句在讲什么。这条规则由代码强制（不只靠可编辑的提示词），因此自定义 system prompt 也适用：
  - **只能写**：情绪、表演方式、语速/咬字 — 一句话以内（≤10 个英文单词 / ≤16 个汉字，逗号分句不超过 3 段）
  - **禁止写**：音色、音区、年龄、性别、口音（这些是角色的固定音色身份，属于 **Character Style**）、动作与手势、场景描述或"这句话表现了什么"、方括号舞台提示、引号、省略号、以及说话人自己的名字
  - 优先用可执行的措辞而非模糊的能量词：本项目实测，明确的语速指令（"语速极慢，拖长音"）在 3 段不同文本上分别让时长增加 10–28%；而"强调事件的诡异"这类无声学目标的导演注释没有稳定效果，因为声音无法执行它
  - 每次生成都会在 Script 页日志里打一行审计，例如 `[instruct audit:final] 17/36 line(s) need attention: scene=8, action=6, no_delivery=5`，并回引有问题的行。格式类问题（多余引号、`旁白：` 前缀、重复标点、超长）会被自动清洗；语义类问题由审计报告，并由复审（Review）修正
