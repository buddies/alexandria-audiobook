...
- **Preview = the real payload** — the character style is always included, exactly as a render would send it, and the "情绪 / 风格" box above the cards adds a per-line Emotion / Style direction on top of it for that preview only (it is not saved). Under the player, the card prints the string that was actually sent (`Sends: …`), so you can verify by ear against a known payload — including how the **Emotion Direction Per Line** policy changes it. Cloned voices print that they send no instructions, since the clone path ignores them
